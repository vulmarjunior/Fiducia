...
export function Categories() {
  ...
  const handleImportDefaults = async () => {
    ...
    const defaultCategories = [
      ...
      { name: 'Bônus', type: 'receita', icon: 'Gift' },
      { name: 'Outros', type: 'receita', icon: 'HelpCircle' }
    ];
    ...
    try {
      ...
      for (const cat of defaultCategories) {
        // Check if category already exists (case insensitive and same type)
        const exists = categories.some(c => 
          c.name.toLowerCase() === cat.name.toLowerCase() && 
          (c.type === cat.type || (cat.type === 'despesa' && c.type === 'expense') || (cat.type === 'receita' && c.type === 'income'))
        );

        if (!exists) {
          const newDocRef = doc(collection(db, 'categories'));
          
          if (orderCounters[cat.type] === undefined) {
            const existingSiblings = categories.filter(c => c.type === cat.type && !c.parentId);
            orderCounters[cat.type] = existingSiblings.length > 0 ? Math.max(...existingSiblings.map(c => c.order || 0)) + 1 : 0;
          }
