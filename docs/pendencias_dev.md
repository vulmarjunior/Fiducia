...
**Estado:** APK diagnóstico compilado em `artifacts/Fiducia-diagnostic-debug.apk`. Shell web, autorização explícita, captura restrita, parser, deduplicação e armazenamento local sanitizado estão implementados. Não há comunicação com Firebase nesta fase.
...
**Pendente:** capturar compras/estornos reais do C6 e Itaú. O acesso especial às notificações já foi concedido e verificado. Até a validação com alertas reais, não criar transações nem candidatos no Firebase, não publicar o APK e não incrementar a versão web `0.15.5`.
